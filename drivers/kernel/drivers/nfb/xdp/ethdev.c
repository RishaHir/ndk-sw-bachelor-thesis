...
void destroy_ethdev(struct nfb_ethdev *ethdev)
{
	...
	nfb_xdp_sysfs_deinit_ethdev(ethdev);
...
struct nfb_ethdev *create_ethdev(struct nfb_xdp *module, int fdt_offset, u16 index)
{
	...
	if (!netdev) {
		printk(KERN_ERR "nfb: Failed to allocate netdevice.\n");
	...
	snprintf(netdev->name, IFNAMSIZ, "nfb%up%u", nfb->minor, index);

	// Initialize nfb_ethdev struct
	ethdev = netdev_priv(netdev);
	ethdev->index = index;
	ethdev->channel_count = module->rxqc / module->ethc;
	ethdev->channel_offset = ethdev->channel_count * index;
	...
	if(nfb_xdp_sysfs_init_ethdev(ethdev)) {
		printk(KERN_ERR "nfb: Failed to init sysfs.\n");
		goto err_sysfs_init;
	}

	ret = register_netdev(netdev); // calls nfb_xdp_open
	if (ret) {
		printk(KERN_ERR "nfb: Failed to register netdevice.\n");
