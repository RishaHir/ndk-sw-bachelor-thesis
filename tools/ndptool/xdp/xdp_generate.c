...
static int xdp_mode_generate_loop(struct ndp_tool_params *p)
{
	...
	// Set constant length if random generation is disabled
	...
	while (!stop) {
		// Check the packet limits if set
		if (limit_packets > 0) {
			if (packets_rem == 0) {
				break;
			}
		...
		// Generate random lengths if set
		if (gen_index != -1) {
			for (i = 0; i < burst_size; i++) {
...
int xdp_mode_generate_check(struct ndp_tool_params *p)
{
	...
	// So that rand(seed) % (delta + 1) randomizes the lengths
	...
	// Map NDK DMA queues to AF_XDP channels 
